...
//Search.prototype.checkMonth = function(offset) {
    ...
    //     uri;
Search.prototype.checkWeek = function(offset) {
    var date = moment().subtract(offset, 'weeks'),
        start = this.formatDate(date.startOf('week')),
        end = this.formatDate(date.endOf('week')),
        uri;
...
Search.prototype.start = function(weeks) {
    this.queue = async.queue(this.requestURI.bind(this), 1);

    while (weeks >= 0) {
        this.checkWeek(weeks--);
    }
}
